...
const createBooking = asyncWrapper(async (req, res, next) => {
  ...
  console.log('📝 Booking creation request:', { customer, vehicle, serviceType, scheduledDate, timeSlot, description, priority });
...
const createBookingPublic = asyncWrapper(async (req, res, next) => {
  const { customer, vehicle, serviceType, scheduledDate, timeSlot, description, priority } = req.body;

  console.log('📝 Public booking creation request:', { customer, vehicle, serviceType, scheduledDate, timeSlot, description, priority });
  ...
  // Create booking without requiring authenticated user
  const bookingData = {
    customer,
    vehicle,
    serviceType,
    ...
    description,
    priority: priority || "medium",
    createdBy: customer, // Use customer as the creator for public bookings
    notes: [{
      note: "Booking created via online form",
      createdBy: customer, // Customer created the note too
      createdAt: new Date()
    }]
  };
  ...
  try {
    const booking = await Booking.create(bookingData);
    console.log('✅ Public booking created successfully:', booking._id);
    ...
    return next(createCustomError(error.message || "Failed to create booking", 400));
  }
});
