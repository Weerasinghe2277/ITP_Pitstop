...
const UserSchema = new mongoose.Schema(
  {
    ...
    employeeDetails: {
      ...
      department: {
        type: String,
        enum: ["mechanical", "electrical", "bodywork", "detailing", "inspection", "customer_service", "management", "front_desk"],
        required: function () {
          return ["technician", "service_advisor", "manager", "cashier"].includes(this.role);
        },
      },
...
UserSchema.pre("save", async function (next) {
  if (this.isNew && !this.userId) {
    try {
      ...
      // More robust userId generation with multiple attempts
      let attempts = 0;
      ...
      while (attempts < maxAttempts) {
        ...
        try {
          // Find the highest existing userId for this role with a more specific regex
          const lastUser = await this.constructor
            .findOne({
              userId: new RegExp(`^${prefix}\\d{5}$`),
              role: this.role
            })
            .sort({ userId: -1 })
            .select("userId")
          ...
          const candidateUserId = `${prefix}${nextNumber.toString().padStart(5, "0")}`;

          // Check if this userId already exists (double-check to avoid race conditions)
          const existingUser = await this.constructor
            .findOne({ userId: candidateUserId })
            .select("_id")
            .lean();
          ...
          if (!existingUser) {
            ...
            console.log(`⚠️ UserId ${candidateUserId} already exists, trying next number...`);
            // Continue to next iteration to try the next number
          }
      ...
      if (!this.userId) {
        throw new Error(`Failed to generate unique userId after ${maxAttempts} attempts`);
      }

    } catch (error) {
      console.error(`❌ Error generating userId for role ${this.role}:`, error.message);
      return next(error);
    }
  }
